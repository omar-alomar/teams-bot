...
export class Planner {
  ...
  constructor(
    ...
    llmApiKey: string = process.env.OPENAI_API_KEY || '',
    llmBaseUrl: string = process.env.OPENAI_BASE_URL || 'https://api.openai.com/v1',
    llmModel: string = process.env.OPENAI_MODEL || 'gpt-4o-mini'
  ) {
  ...
  async plan(message: string, userId: string, context: any = {}, conversationHistory: Message[] = []): Promise<Plan> {
    // Get context
...
- NEVER return an empty calls array for information requests - always try to find relevant tools`;
  ...
  private async callLLM(prompt: string, conversationHistory: Message[] = []): Promise<string> {
    ...
    try {
      ...
      throw new Error(
        `Failed to connect to LLM API at ${url}. ` +
