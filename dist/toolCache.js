...
class ToolCache {
    ...
    generateExamples(toolName, description, schema) {
        const examples = [];
        // Generate examples based on tool name and common patterns
        if (toolName === 'send_document') {
            examples.push({ query: 'send me the ecp checklist', args: { query: 'ecp checklist' } }, { query: 'where is the submittal package', args: { query: 'submittal package' } }, { query: 'get me the project deadline document', args: { query: 'project deadline' } });
        }
        else if (toolName.includes('project') || toolName.includes('project')) {
            examples.push({ query: 'show me project details', args: {} }, { query: 'what projects do we have', args: {} });
        }
        else if (toolName.includes('user') || toolName.includes('users')) {
            examples.push({ query: 'list all users', args: {} }, { query: 'who are the team members', args: {} });
        }
        else if (toolName.includes('document') || toolName.includes('doc')) {
            examples.push({ query: 'get document by id', args: { id: 'example-id' } }, { query: 'find document', args: { query: 'example' } });
        }
        else if (toolName.includes('client')) {
            examples.push({ query: 'get client information', args: {} }, { query: 'show me client details', args: {} });
        }
        else if (toolName.includes('phone') || toolName.includes('phone_number')) {
            examples.push({ query: 'get phone number for client', args: {} }, { query: 'what is the phone number', args: {} }, { query: 'send me phone number', args: {} }, { query: 'send me burkards number', args: {} }, { query: 'get burkards phone', args: {} }, { query: 'phone number', args: {} });
        }
        else if (toolName.includes('email')) {
            examples.push({ query: 'get email address for client', args: {} }, { query: 'what is the email', args: {} }, { query: 'send me email', args: {} }, { query: 'send me burkards email', args: {} }, { query: 'get bruce harveys email', args: {} }, { query: 'email address', args: {} });
        }
        else if (toolName.includes('address')) {
            examples.push({ query: 'get address for client', args: {} }, { query: 'what is the address', args: {} }, { query: 'send me address', args: {} }, { query: 'address', args: {} });
        }
    ...
    analyzeSafety(toolName, description, schema) {
        const lowerName = toolName.toLowerCase();
        const lowerDesc = description.toLowerCase();
        // Check for destructive operations
        const destructiveKeywords = ['delete', 'remove', 'destroy', 'drop', 'clear', 'truncate', 'cancel'];
        const destructive = destructiveKeywords.some(kw => lowerName.includes(kw) || lowerDesc.includes(kw));
        // Check for write operations
        const writeKeywords = ['create', 'update', 'edit', 'modify', 'add', 'insert', 'save', 'set', 'post', 'put', 'patch'];
        const write = writeKeywords.some(kw => lowerName.includes(kw) || lowerDesc.includes(kw)) || destructive;
