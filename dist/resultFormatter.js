...
class ResultFormatter {
    constructor(filesBaseUrl = process.env.FILES_BASE_URL || 'https://files.mba-eng.com') {
        this.filesBaseUrl = filesBaseUrl;
    }
    async format(result, toolName) {
    ...
    async formatContentArray(content, toolName) {
        const formatted = {
            text: '',
            attachments: [],
        };
        for (const item of content) {
            if (item.type === 'blob') {
                const fileCard = this.createFileCard(item.name || `file_${Date.now()}.pdf`, item.mimeType || 'application/pdf', item.data ? `data:${item.mimeType || 'application/pdf'};base64,${item.data}` : undefined, undefined, item.data ? Buffer.from(item.data, 'base64').length : undefined);
                ...
                try {
                    ...
                    if (parsed.downloadUrl || parsed.filename) {
                        const fileCard = this.createFileCard(parsed.filename || parsed.name || 'document', parsed.contentType || 'application/pdf', parsed.downloadUrl, parsed.message, parsed.size);
                        formatted.attachments?.push(fileCard);
                    }
    ...
    formatFileResult(result) {
        const card = this.createFileCard(result.filename || result.name || 'document', result.contentType || 'application/pdf', result.downloadUrl, result.message, result.size);
        return {
            attachments: [card],
        };
    }
            ...
            for (const row of displayRows) {
                const values = headers.map(h => {
                    ...
                    if (typeof val === 'object')
                        return JSON.stringify(val).slice(0, 50); // Increased from 30 to 50
    ...
    formatObjectResult(result, toolName) {
    ...
    formatSimpleResult(result, toolName) {
    ...
    createFileCard(filename, contentType, downloadUrl, message, size) {
